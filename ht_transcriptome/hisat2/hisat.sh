...
me="hisat::${PBS_JOBNAME}::$RUN_STEP on $HOSTNAME"
...
if [ "${RUN_STEP}" == "hisat" ] ; then
    ...
    for left_file in ${array1[@]}; do
        ...
        #echo starting hisat2 --dta -x ${cvir_genome}/cvir_edited -1 ${left_file} -2 ${right_file} -S ${trans2017}/${file}.sam
        echo starting hisat2 --dta -x ${bac_genomes}/allgenomes_bac_index -1 ${left_file} -2 ${right_file} -S ${trans2017}/bac/${file}.sam
        #post_slack_message cluster-jobs "starting hisat2 --dta -x ${cvir_genome}/cvir_edited_index -1 ${left_file} -2 ${right_file} -S ${trans2017}/${file}.sam" "${me}.${file}"
...
if [ "${RUN_STEP}" == "samtools_stats" ] ; then
    array3=($(ls -1 ${trans2017}/${FILES[$PBS_ARRAYID]}.bam))
    for i in ${array3[@]}; do
        base=$(basename $i)
